...
/**
 * Storefront subnavbar (menu leaf) product listing.
 *
 * Response shape matches the legacy /v1/subnavbar/{slug}/products endpoint,
...
class SubnavbarController extends Controller
{
    ...
    public function products(string $slug, Request $request): JsonResponse
    {
        ...
        $subnavbarQuery = SubnavbarItem::where('slug', $slug)->where('status', 'active');

        StorefrontScope::apply($subnavbarQuery);

        $subnavbarItem = $subnavbarQuery->first();
        ...
        // Apply sorting. Price sorts use the discounted price so the order
        // matches the discounted prices shown in the UI.
        switch ($sort) {
            case 'price_asc':
                $query->orderBy(

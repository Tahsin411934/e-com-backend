// ============================================================
// Auth Server Actions
// The backend returns a token; the Next.js server action stores
// it in an httpOnly cookie so middleware and server components
// can authenticate without localStorage.
// ============================================================
...
async function deleteTokenCookie(): Promise<void> {
  const cookieStore = await cookies();
  cookieStore.delete(TOKEN_COOKIE_NAME, { path: "/" });
}
...
export async function loginAction(
  ...
  try {
    const data = await apiRequest<{ status: string; message: string; user?: any; token?: string }>('/api/login', {
      method: 'POST',
      body: JSON.stringify({
        email: formData.get('email'),
        password: formData.get('password'),
    ...
    const redirectTo = (formData.get('redirect') as string | null)?.trim() || '/dashboard';
    redirect(redirectTo);
    return {
      success: true,
